...
def append_posthoc_calibration_sheet(workbook: Workbook) -> None:
    ...
    sheet["A2"] = "样本来源"
    sheet["B2"] = "单选/总体规则来自四期复盘：1366场入选，1362场完赛；双选v2来自五期补充复盘：2005场完赛，中信任双选986场、主客对冲308场、带平双选678场。"
    sheet["A3"] = "总体命中"
    sheet["B3"] = "四期口径：高信任单选 143/223=64.1%；中信任单选 226/457=49.5%；中信任双选 508/682=74.5%。"
    sheet["A4"] = "使用边界"
    sheet["B4"] = "校准层只作辅助风险修正，不改写原模型结果，生产报表不抓取实际赛果。平局不作为进攻型单选预测，只作为保护、结构替换或回避信号；市场共识>0.86不再单独触发优先第二个。"
    ...
    current_row += 1
    double_rows = [
        ["主客对冲双选 + 前二差值<0.06", "改第二项/平局", "第二项/平局", "中", "五期回测改第二项+平局约76.3%，高于原对冲66.9%；建议降一档仓位。"],
        ["主客对冲双选 + 前二差值>=0.06", "保留原双选", "原双选", "中", "该区间原对冲约71.6%，优于第二项+平局58.6%；但对冲类整体平局率约31%，仓位低于带平双选。"],
        ["非对冲双选 + 前二差值<0.02", "优先第二个", "第二项", "中", "第二项优势最集中，五期第二项命中46.0%，高于第一项28.6%。"],
        ["非对冲双选 + 前二差值0.02~0.03", "保留原双选", "原双选", "中", "跨期不稳定，作为缓冲区保留双选，禁止单选化。"],
        ["非对冲双选 + 前二差值>=0.03", "优先第一个", "第一项", "中", "不再按市场共识或组合文本强行优先第二个；差值>=0.06时第一项优势更明确。"],
        ["平局排第一且未触发<0.02", "保留原双选", "原双选", "中", "避免把平局作为进攻型单选；平局只用于防守保护。"],
        ["市场共识>0.86", "不单独触发", "按前二差值/对冲规则", "中", "五期补充显示共识在小差值段不单调，不再作为优先第二个的独立条件。"],
        ["旧式逐场平局保护/警惕主胜", "撤销", "不输出", "中", "逐场平局预警未显示有效行级区分能力，改为对冲双选的结构替换。"],
    ]
    ...
    current_row += 2
    sheet.cell(current_row, 1, "双选v2监控口径")
    sheet.cell(current_row, 1).font = Font(bold=True)
    sheet.cell(current_row, 1).fill = section_fill()
    current_row += 1
    monitor_rows = [
        ["gap<0.02 第二项优势", "若连续2期优势<+5pp，降级为保留双选。"],
        ["gap 0.02~0.03 缓冲区", "继续跟踪保留双选命中，暂不单选化。"],
        ["对冲gap<0.06 第二项/平局", "若单期输原对冲>=10pp或连续2期为负，恢复为保留原对冲。"],
        ["总体平局基线", "用于判断平局保护是否仍高于自然平局率。"],
    ]
