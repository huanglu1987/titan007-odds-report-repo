...
def append_posthoc_calibration_sheet(workbook: Workbook) -> None:
    ...
    sheet["A2"] = "样本来源"
    sheet["B2"] = "九期样本外复现检验：3943场完赛，17场推迟/改期未匹配；中信任双选2000场。防守型单选规则参考带校准列7期3667场完赛、3211场可下注。"
    sheet["A3"] = "总体命中"
    sheet["B3"] = "九期双选排序：gap<0.03第二项优势+10.6pp，7/9期为正；gap>=0.06第一项优势约+12.5pp；对冲改第二项/平局部署63% vs 原双选76%（-13pp），已撤回。"
    sheet["A4"] = "使用边界"
    sheet["B4"] = "校准层只作辅助风险修正，不改写原模型结果，生产报表不抓取实际赛果。gap<0.10保留双选属于命中率/赔率覆盖面交换，未固化为默认规则，需用赔率EV再验收。"

    current_row = 6
    sheet.cell(current_row, 1, "双选规则v3")
    ...
    current_row += 1
    double_rows = [
        ["非对冲双选 + 前二差值<0.03", "优先第二个", "第二项", "中", "九期合并第二项优势+10.6pp，样本外4期+6.7pp；撤销0.02细分和缓冲带。"],
        ["非对冲双选 + 前二差值>=0.03", "优先第一个", "第一项", "中", "0.03是九期硬反转点；gap>=0.06时第一项优势约+12.5pp。"],
        ["平局排第一 + 前二差值>=0.03", "保留原双选", "原双选", "中", "避免把平局作为进攻型单选；平局只用于防守保护。"],
        ["主客对冲双选", "保留原双选", "原双选", "中", "撤回v2平局改造：部署后63%低于原双选76%（-13pp）；对冲平局率29%接近全体26%。"],
        ["市场共识>0.86", "不单独触发", "按前二差值规则", "中", "九期复现未支持高共识作为独立选第二条件。"],
        ["gap<0.10保留双选", "参数观察", "EV待验证", "中", "命中率口径可提升但属于覆盖面/赔率交换，不作为当前生产默认动作。"],
    ]
    ...
    current_row += 2
    sheet.cell(current_row, 1, "v3监控口径")
    sheet.cell(current_row, 1).font = Font(bold=True)
    sheet.cell(current_row, 1).fill = section_fill()
    current_row += 1
    monitor_rows = [
        ["gap<0.03 第二项优势", "下期固定跟踪第二项 vs 第一项；若连续2期优势消失，再降级为保留双选。"],
        ["gap>=0.06 第一项优势", "确认第一项优势是否仍维持约+12pp。"],
        ["对冲双选平局率", "确认是否仍接近全体平局基率；未出现超额前不恢复平局改造。"],
        ["防守型四规则净效果", "继续跟踪谨慎防平、补平或回避、改带平双选、建议回避是否仍为正。"],
    ]
    for name, basis in monitor_rows:
        sheet.append([name, "", "", "", basis])
        current_row += 1
    ...
    single_rows = [
        ...
        ["主胜单 + 前二差值0.45~0.55 + 共识<0.84", "改带平双选", "主胜/平局", "高", "部署后31场命中87%，高于同场原预测71%，防守型补平规则强复现。"],
        ["结构=谨慎-主客胶着", "回避", "不跟", "高", "九期参数优化中剩余单选池命中约31.4%，稳定弱结构。"],
        ["结构=高-防平", "回避", "不跟", "高", "九期参数优化中单选命中约35.5%，远低于池均。"],
        ...
        ["客胜单 + 前二差值<0.5", "谨慎补平或回避", "客胜/平局", "中", "部署后25场校准命中72%，高于同场原预测40%。"],
        ["结构=中-偏平", "观察降权", "不单独改写", "中", "九期参数优化中命中约43.0%，暂作弱结构观察，不一刀切回避。"],
        ["其他中信任/弱单选", "谨慎防平", "原单选/平局", "中", "部署后109场校准命中73%，高于同场原预测44%，防守型规则强复现。"],
    ]
