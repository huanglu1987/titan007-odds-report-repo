...
- 报表会额外输出“赛后校准/风险修正建议”；这是辅助规则层，不改写原模型的最终预测。当前采用九期样本外复现 v3 口径（3943 场完赛、中信任双选 2000 场），并保留防守型单选规则的独立追踪。
...
- `赛后校准动作`：基于赛后复盘规则给出的辅助动作；双选采用九期 v3 规则，单选重点保留防平、补平和回避等防守型规则，例如“建议跟随”“优先第一个”“优先第二个”“保留原双选”“建议回避”
- `校准建议结果`：校准层建议关注的结果，不等于原模型的 `最终预测结果`
- `校准风险等级`：按复盘规则标记为低/中/高风险
- `校准依据`：说明触发的复盘规则和历史样本提示；双选依据会区分九期 v3 的 0.03 排序开关、主客对冲撤回平局改造、以及 gap<0.10 的 EV 待验证观察项
...
- `赛后校准/风险修正建议` 是辅助判断层，不代表模型原结果被替换；九期 v3 确认 `前二差值<0.03` 优先第二个、`>=0.03` 优先第一个；撤销 v2 的 `0.02` 细分、`0.02~0.03` 缓冲带和主客对冲“第二项/平局”改造，主客对冲双选默认保留原双选。平局只作为防守保护，不作为进攻型单选预测；防守型单选规则（谨慎防平、补平或回避、改带平双选、建议回避）是校准层重点。
- `gap<0.10` 保留双选目前只写入规则页作为命中率/赔率覆盖面交换的观察参数；未引入赔率 EV 验证前，不作为生产默认动作。
