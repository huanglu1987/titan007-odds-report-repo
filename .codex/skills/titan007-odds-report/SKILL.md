...
- 报表会额外输出“赛后校准/风险修正建议”；这是辅助规则层，不改写原模型的最终预测。单选沿用四期 1366 场入选、1362 场完赛复盘口径；双选采用五期补充复盘 v2 口径（2005 场完赛、中信任双选 986 场）。
...
- `赛后校准动作`：基于赛后复盘规则给出的辅助动作；单选沿用四期规则，双选采用五期 v2 规则，例如“建议跟随”“优先第一个”“优先第二个”“保留原双选”“改第二项/平局”“建议回避”
- `校准建议结果`：校准层建议关注的结果，不等于原模型的 `最终预测结果`
- `校准风险等级`：按复盘规则标记为低/中/高风险
- `校准依据`：说明触发的复盘规则和历史样本提示；双选依据会区分五期 v2 的极小差值、缓冲区和主客对冲结构替换
...
- `赛后校准/风险修正建议` 是辅助判断层，不代表模型原结果被替换；单选继续使用四期版信任分级、女足/高差值/高共识加分、防平补平与回避规则。双选使用五期 v2 规则：`前二差值<0.02` 才强触发优先第二个，`0.02~0.03` 保留双选，主客对冲且差值<0.06改为“第二项/平局”。平局只作为防守保护或结构替换，不作为进攻型单选预测；市场共识>0.86不再独立触发优先第二个，旧版“改选平局/警惕主胜”等改方向规则已撤销。
