...
- 报表会额外输出“赛后校准/风险修正建议”；这是基于 2026-05-16 至 2026-05-18 两期复盘样本的辅助层，不改写原模型的最终预测。
...
- `赛后校准动作`：基于复盘规则给出的辅助动作，例如“建议跟随”“优先第一个”“建议改带平双选”“建议回避”
- `校准建议结果`：校准层建议关注的结果，不等于原模型的 `最终预测结果`
- `校准风险等级`：按复盘规则标记为低/中/高风险
- `校准依据`：说明触发的复盘规则和历史样本提示
...
- `赛后校准/风险修正建议` 是辅助判断层，不代表模型原结果被替换；引用或下单前应同时查看 `最终预测结果` 与校准风险。
