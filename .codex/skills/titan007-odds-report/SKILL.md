...
- 报表会额外输出“赛后校准/风险修正建议”；这是基于四期 1366 场入选、1362 场完赛复盘样本的辅助层，不改写原模型的最终预测。
...
- `赛后校准动作`：基于四期汇总复盘规则给出的辅助动作，例如“建议跟随”“优先第一个”“优先第二个”“建议改带平双选”“建议回避”
- `校准建议结果`：校准层建议关注的结果，不等于原模型的 `最终预测结果`
- `校准风险等级`：按复盘规则标记为低/中/高风险
- `校准依据`：说明触发的四期复盘规则和历史样本提示
...
- `赛后校准/风险修正建议` 是辅助判断层，不代表模型原结果被替换；四期版确认平局只作为防守保护，不作为进攻型单选预测，旧版“改选平局/警惕主胜”等改方向规则已撤销。
